...
export const updateOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!validTransitions[currentStatus] || !validTransitions[currentStatus].includes(newStatus)) {
    return next(new ErrorHandler(`Invalid status transition from ${currentStatus} to ${newStatus}`, 400));
  }
  ...
  if (currentStatus === "Processing" && newStatus === "Shipped") {
    // Update products stock only for "Processing" to "Shipped" transition
  ...
  order.deliveredAt = newStatus === "Delivered" ? Date.now() : null;
